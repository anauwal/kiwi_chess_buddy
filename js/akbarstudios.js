        ...
        if (bestMoveSelected && this.selfmaster.options.legit_auto_move && this.selfmaster.game.controller.getPlayingAs() === this.selfmaster.game.controller.getTurn()) {
            ...
            if (window.toaster) {
                window.toaster.add({
                    ...
                    icon: "circle-info",
                    content: `Akbarstudios: Auto move in ${secondsTillAutoMove} seconds`,
                    // autoClose: 3000,
                    style: {
                        position: "fixed",
                        bottom: "60px",
...
class AkbarStudios {
    constructor(chessboard, options) {
        ...
        window.addEventListener("AkbarStudiosUpdateOptions", (event) => {
            ...
            if (window.toaster && window.toaster.notifications.findIndex((noti) => noti.id == "akbarstudios-settings-updated") == -1) {
                window
                    .toaster
                    .add(
                        {id: "akbarstudios-settings-updated", duration: 2000, icon: "circle-gearwheel", content: `Settings updated!`}
                    );
            }
        }, false);
    }
    onEngineLoaded() {
        if (window.toaster) {
            window
                .toaster
                .add(
                    {id: "chess.com", duration: 3000, icon: "circle-info", content: `Match is started!!`}
                );
        }
    }
}
...
"AkbarStudiosGetOptions" and is expected to be handled by an event listener that will send a response
event named "AkbarStudiosSendOptions" with the requested data. The `requestId` variable is used to
        ...
        return new Promise(function (resolve, reject) {
            var listener = function (evt) {
                if (evt.detail.requestId == id) {
                    // Deregister self
                    window.removeEventListener("AkbarStudiosSendOptions", listener);
                    resolve(evt.detail.data);
                }
            };
            window.addEventListener("AkbarStudiosSendOptions", listener);
            var payload = {
                data: data,
                id: id
            };
            window.dispatchEvent(
                new CustomEvent("AkbarStudiosGetOptions", {detail: payload})
            );
        });
    }
    return {getData: getData};
})();
function InitAkbarStudios(chessboard) {
    ...
    ChromeRequest.getData().then(function (options) {
            try {
                selfmaster = new AkbarStudios(chessboard, options);
            } catch (e) {
...
window.onmessage = function (event) {
    ...
    <span id="bmtitle">AkbarStudios</span>
    </div>
    </div>
    `
        document
